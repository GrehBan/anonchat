...
from anonchat.application.message.handlers.redis import ProcessRedisMessageEvent
...
class MessageStreamWorker(RedisWorker):
    ...
    async def consume(self) -> None:
        ...
        self._running = True
        self._logger.info(f"started. Listening: {self._stream_key}")
        ...
        while self._running:
            try:
                ...
                async with self._session_maker() as session:
                    handler = ProcessRedisMessageEvent()
                    for stream, messages in events:
                        for msg_id, data in messages:
                            try:
                                async with SqlalchemyMessageUoW(session) as uow:
                                    await handler.handle(data, uow)
                                    await uow.commit()
                            except Exception as e:
                                self._logger.error(f"Error processing message {msg_id}", e, exc_info=True)
